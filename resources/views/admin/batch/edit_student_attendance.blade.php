...
<div class="grid grid-cols-12 gap-6 mt-5">
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-hide">
        <form method="post" action="{{ route('admin.saveEditedAttendance') }}">  
        <table class="table table-report -mt-2" id="userAttendance">
            ...
            <input type="hidden" value="{{ json_encode($presetUserId) }}" name="presetUserId">
            ...
            @if(!is_null($getStudent))
            	@foreach($getStudent as $fk => $fv)
	                <tr class="intro-x">
                        ...
                        <td class="text-center">
                            <input class="form-check-switch ml-auto users" type="checkbox" name="user_id[]" value="{{ $fv->student_id }}" @if(in_array($fv->student_id,$presetUserId)) checked="checked" @endif>
                        </td>
	                </tr>
	            @endforeach
	        @endif
